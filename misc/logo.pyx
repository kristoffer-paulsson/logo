...
class Logo(App):
    ...
    def __init__(self, **kvargs):
        super(Logo, self).__init__(**kvargs)
        ...
        self.__dir = os.path.dirname(self.directory)
        self.dict_language = literal_eval(
            open(os.path.join(
                self.__dir, 'assets', 'locales', 'locales.txt')).read())
        self.translation = Translation(
            self.lang, 'Ttest', os.path.join(self.__dir, 'assets', 'locales')
        )

    def get_application_config(self):
        return super(Logo, self).get_application_config(
                        '{}/%(appname)s.ini'.format(self.__dir))

    def build_config(self, config):
        '''Создаёт файл настроек приложения logo.ini.'''

        config.adddefaultsection('General')
        config.setdefault('General', 'language', 'en')

    def set_value_from_config(self):
        '''Устанавливает значения переменных из файла настроек logo.ini.'''

        self.config.read(os.path.join(self.__dir, 'logo.ini'))
        self.lang = self.config.get('General', 'language')

    def build(self):
        self.set_value_from_config()
        self.load_all_kv_files(
            os.path.join(self.__dir, 'lib', 'logo', 'kv'))
    ...
    def show_about(self, *args):
        ...
        self.screen.ids.about.ids.label.text = \
            self.translation._(
                u'[size=20][b]Logo[/b][/size]\n\n'
                u'[b]Version:[/b] {version}\n'
                u'[b]License:[/b] MIT\n\n'
                u'[size=20][b]Developer[/b][/size]\n\n'
                u'[ref=SITE_PROJECT]'
                u'[color={link_color}]Kristoffer Paulsson[/color][/ref]\n\n'
                u'[b]Source code:[/b] '
                u'[ref=https://github.com/kristoffer-paulsson/logo.git]'

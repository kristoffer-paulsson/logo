...
def start():
    ...
    # If this name is found in environ, Kivy will not read the user config
    # file.
    ...
    # Config object. They are loaded only once when kivy is imported. The
    # behavior can be disabled using KIVY_NO_ENV_CONFIG.
    # os.environ['KCFG_section_key'] = ''

    # If set, no environment key will be mapped to configuration object. If
    # unset, any KCFG_section_key=value will be mapped to Config.
    ...
    # Whether the GLES2 restrictions are enforced
    # (the default, or if set to 1). If set to false, Kivy will not be truly
    # GLES2 compatible. Following is a list of the potential incompatibilities
    # that result when set to true.
    ...
    if getattr(sys, "frozen", False):  # Bundle mode with PyInstaller
        os.environ["LOGO_ROOT"] = sys._MEIPASS
    else:
        # sys.path.append(os.path.abspath(__file__).split("demos")[0])
        os.environ["LOGO_ROOT"] = os.path.dirname(
            os.path.abspath(os.path.join(os.curdir, "logo")))

    os.environ["LOGO_ASSETS"] = os.path.join(
        os.environ["LOGO_ROOT"], f"assets{os.sep}")
